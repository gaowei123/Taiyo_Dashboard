...
namespace Common.Class.DAL
{
    public class AssyUserDB
    {
        ...
        public DataTable GetList(string UserID, string password)
        {
            ...
            DataSet ds = DBHelp.SqlDB.Query(strSql.ToString(), parameters, DBHelp.Connection.SqlServer.SqlConn_Assy_Server);
        ...
        internal DataTable GetUserList()
        {
            ...
            strSql.Append("  where USER_GROUP <> 'Admin'   ");
            strSql.Append("  order by USER_GROUP, UserID");

            DataSet ds = DBHelp.SqlDB.Query(strSql.ToString(), DBHelp.Connection.SqlServer.SqlConn_Assy_Server);
