...
namespace Common.Class.BLL
{
	...
	/// </summary>
	public class PQCQaViDetailHistory_BLL
	{
		private readonly Common.DAL.PQCQaViDetailHistory_DAL dal=new Common.DAL.PQCQaViDetailHistory_DAL();
		public PQCQaViDetailHistory_BLL()
		{}
		#region  Method
		...
		/// 增加一条数据
		/// </summary>
		public SqlCommand AddCommand(Common.Model.PQCQaViDetailHistory_Model model,SqlCommand cmd=null)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Common.Model.PQCQaViDetailHistory_Model> GetModelList(string strWhere)
		{
        ...
        /// 获得数据列表
        /// </summary>
        public List<Common.Model.PQCQaViDetailHistory_Model> DataTableToList(DataTable dt)
        {
            List<Common.Model.PQCQaViDetailHistory_Model> modelList = new List<Common.Model.PQCQaViDetailHistory_Model>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                Common.Model.PQCQaViDetailHistory_Model model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new Common.Model.PQCQaViDetailHistory_Model();
                    ...
                    if (dt.Rows[n]["passQty"].ToString() != "")
                    {
                        model.passQty = decimal.Parse(dt.Rows[n]["passQty"].ToString());
                    }
		...
		/// 获得前几行数据
		/// </summary>
		public Common.Model.PQCQaViDetailHistory_Model CopyObj(Common.Class.Model.PQCQaViDetailTracking_Model objModel )
		{
			Common.Model.PQCQaViDetailHistory_Model model;
			model = new Common.Model.PQCQaViDetailHistory_Model();
            ...
            model.totalQty = objModel.totalQty;
            model.totalPassQty = objModel.totalPassQty;
            model.totalRejectQty = objModel.totalRejectQty;
            model.passQty = objModel.passQty;
